...
bool SPI_FIFO_t3::begin(SPISettings settings,bool avoidInit)
{
	...
	} else if ((_mosi == 0) && (_sclk == 32) && (_miso == 255 || _miso == 1)){
		_spiBus = 1;
...
void SPI_FIFO_t3::clearFifoBuffer(void)
{
	#if defined(__MK20DX128__) || defined(__MK20DX256__)
...
uint8_t SPI_FIFO_t3::readByte_cont(void)
{
...
uint16_t SPI_FIFO_t3::readWord_cont(void)
{
	uint8_t dummy __attribute__((unused));
	uint8_t a = 0;
	uint8_t b = 0;
